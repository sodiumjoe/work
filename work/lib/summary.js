...
const { VAULT_ROOT, PLAN_DIR, PROJECT_DIR, notePath } = require('./paths.js');
...
function collectWeekEntries(dateStr) {
  ...
  if (fs.existsSync(PROJECT_DIR)) {
    ...
    for (const f of files) {
      ...
      const entries = changelog.filter(l => {
        const match = l.match(datePattern);
        return match && dates.has(match[1]);
      });
      if (entries.length > 0) {
        groups.push({ title, sourceType: 'project', entries });
      }
    }
  }
  ...
  if (fs.existsSync(PLAN_DIR)) {
    ...
    for (const f of files) {
      ...
      if (entries.length > 0) {
        groups.push({ title, sourceType: 'plan', entries });
      }
    }
  }
...
function buildWeeklyPrompt(weekLabel, groups, dailySummaries, outputPath) {
  ...
  for (const g of groups) {
    const prefix = g.sourceType === 'plan' ? '(no project) ' : '';
    parts.push('', `### ${prefix}${g.title}`);
  ...
  parts.push('- Any patterns worth noting (e.g., most of the week on one project, context-switching)');
  parts.push('');
  parts.push('Write in a dry, factual tone. No enthusiasm. No filler. Complete sentences.');
  parts.push(`Use markdown. Start with "# ${weekLabel} Work Summary" as the heading.`);
  return parts.join('\n');
}
...
module.exports = { isoWeek, weekRange, collectWeekEntries, collectDailySummaries, buildWeeklyPrompt, writeWeeklySummary };