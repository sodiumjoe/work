...
## Notes`);
    ...
    const result = fs.readFileSync(path.join(tmpDir, 'projects', 'no-date.md'), 'utf-8');
    assert.ok(result.includes('Done task') === false || result.includes('✅ 2026-03-10'));
    const { extractSection } = require('../lib/markdown.js');
    const changelog = extractSection(result, 'Changelog');
...
## Notes`);
    ...
    const result = fs.readFileSync(path.join(tmpDir, 'projects', 'has-date.md'), 'utf-8');
